...
Scenario: Redo a transaction that installed a group
  Given I execute dnf with args "remove lame"
   Then the exit code is 0
...
Scenario: Redo a transaction that removed a group
  Given I execute dnf with args "group remove DNF-CI-Testgroup"
  Given I execute dnf with args "install lame"
   Then the exit code is 0
...
Scenario: Redo a transaction with a missing group
  Given I drop repository "dnf-ci-thirdparty"
   ...
   Then the exit code is 1
    And stderr contains "Error: The following problems occurred while running a transaction:"
    And stderr contains "  Group id 'dnf-ci-testgroup' is not available."
