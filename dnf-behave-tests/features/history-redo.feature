...
Scenario: Redo a transaction that upgraded a package but the package is downgraded on the system
  ...
  Given I use repository "dnf-ci-thirdparty"
    And I execute dnf with args "install glibc-2.3.1-10"
    And I execute dnf with args "upgrade glibc-2.28-26.fc29"
    And I execute dnf with args "downgrade glibc-2.28-9.fc29"
    And I execute dnf with args "history redo last-1"
   Then the exit code is 0
    And Transaction is following
        | Action        | Package                                    |
...
Scenario: Redo a transaction that installed a package and the package is still on the system
  Given I execute dnf with args "install glibc"
   When I execute dnf with args "history redo last"
   Then the exit code is 0
    And Transaction is empty
